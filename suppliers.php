...
if ( strlen ( $ act ) ==  0  ||  $ act  ==  " del "  ||  $ act  ==  " edit " ) {
  ...
  $ R  =  mysql_db_query ( $ DB , " * เลือกจากซัพพลายเออร์" ) หรือ ตาย ( " phpMyAdmin - "  .  $ create_table_sql  .  " <br/> "  .  mysql_error ());
  echo  " <table> " ;
  while ( $ o  =  mysql_fetch_object ( $ r )) {
    ...
    echo  " <tr> <form action = '' method = post>
      <td> <input ชื่อ id = size = 5 คุ้มค่า = ' " $ o -> SupplierID . " 'style =' สีพื้นหลัง: #dddddd' อ่านได้อย่างเดียว> </ td>   
...
#ส่วนเพิ่มข้อมูล
if ( $ act  ==  " add " ) {
  $ Q   =  "แทรกลงในซัพพลายเออร์ (SupplierID, ContractName, CompanyName) ค่า ( ' " $ _REQUEST { ' ID ' } . " ', ' " $ _REQUEST { ' NS ' } . " ',' " . $ _REQUEST { 'เงินเดือน' } . " ') " ;      
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
...
#ส่วนลบข้อมูล
if ( $ act  ==  " del: confirm " ) {
  $ Q   =  "ลบจากซัพพลายเออร์ที่ SupplierID =' "  $ _REQUEST { ' ID ' }  " ' " ;
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
  $ r  =  mysql_db_query ( db $ , $ q );   
...
# section แก้ไขข้อมูล
if ( $ act  ==  " edit: confirm " ) {
  $ Q   =  "ปรับปรุง$ tbชุด CompanyName =' "  $ _REQUEST { ' NS ' }  " 'ContractName = "  .  $ _REQUEST { 'เงินเดือน' }  " 'ที่ SupplierID = "  .  $ _REQUEST { ' id ' };
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
  $ r  =  mysql_db_query ( db $ , $ q );   
