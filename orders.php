...
if ( strlen ( $ act ) ==  0  ||  $ act  ==  " del "  ||  $ act  ==  " edit " ) {
  ...
  $ R  =  mysql_db_query ( $ DB , " * เลือกจากคำสั่งซื้อ" ) หรือ ตาย ( " phpMyAdmin - "  .  $ create_table_sql  .  " <br/> "  .  mysql_error ());
  echo  " <table> " ;
  while ( $ o  =  mysql_fetch_object ( $ r )) {
    ...
    echo  " <tr> <form action = '' method = post>
      ...
      <td> <ใส่ชื่อ = ขนาด NS = 40 คุ้มค่า = ' " $ o -> ลูกค้า. " ' $ CHG '> </ td>   
      <td> <ใส่ชื่อ = เงินเดือน size = 20 คุ้มค่า =' " $ o -> EmployeeID . " ' $ CHG ; text-align: ถูกต้อง'> </ td>   
...
#ส่วนเพิ่มข้อมูล
if ( $ act  ==  " add " ) {
  $ Q   =  "แทรกลงในคำสั่งซื้อ (OrderID, ลูกค้า, EmployeeID) ค่า ( ' " $ _REQUEST { ' ID ' } . " ', ' " $ _REQUEST { ' NS ' } . " ',' " . $ _REQUEST { 'เงินเดือน' } . " ') " ;      
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
...
#ส่วนลบข้อมูล
if ( $ act  ==  " del: confirm " ) {
  $ Q   =  "ลบออกจากคำสั่งซื้อที่ OrderID =' "  $ _REQUEST { ' ID ' }  " ' " ;
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
  $ r  =  mysql_db_query ( db $ , $ q );   
...
# section แก้ไขข้อมูล
if ( $ act  ==  " edit: confirm " ) {
  $ Q   =  "ปรับปรุง$ tbตั้ง CustomerID =' "  $ _REQUEST { ' NS ' }  " 'EmployeeID =' "  $ _REQUEST { 'เงินเดือน' }  " 'ที่ OrderID = "  .  $ _REQUEST { ' id ' };
  $ conn  =  mysql_connect ( " $ host " , " $ user " , " $ password " );
  $ r  =  mysql_db_query ( db $ , $ q );   
